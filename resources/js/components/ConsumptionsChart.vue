...
const { isFetching, data, error } = useFetch(`/api/contracts/${props.contractId}/consumptions`)
  .get()
  .json();
...
<template>
  <div class="h-64 w-full">
    ...
    <div v-else-if="error" class="text-red-500">Erreur lors du chargement des données.</div>
    <div v-else-if="data && data.length === 0" class="text-sm text-gray-500">
